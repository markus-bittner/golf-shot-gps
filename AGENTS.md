...
- Before every commit that changes the application build, increment `APP_RELEASE.patch` by exactly 1 and update `APP_RELEASE.date` to that build's date using English `DD MMM YYYY` format.
- For every application build, also change the internal `APP_BUILD` value and keep it identical to the `build` value in `build.json`; this internal identifier drives cache-safe update checks and is independent of the user-facing release version.
- Never increment `APP_RELEASE.major` or `APP_RELEASE.subversion`, reset the patch version, or change to a new major/minor version unless the user explicitly instructs it. Documentation-only commits that do not change the application build do not require a version increment.
